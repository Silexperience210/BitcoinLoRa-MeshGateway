...
class BitcoinMeshGateway:
    ...
    def load_config(self):
        """Charge la configuration depuis gateway.ini"""
        config = configparser.ConfigParser()
        config_path = os.path.join(os.path.dirname(__file__), "gateway.ini")
        ...
        if os.path.exists(config_path):
            try:
                ...
                # Configuration mesh
                if config.has_option('mesh', 'port'):
                    port = config.get('mesh', 'port').strip()
                    if port:
                        self.port_var.set(port)
                
                # Configuration Bitcoin
                if config.has_option('bitcoin', 'api'):
                    self.api_var.set(config.get('bitcoin', 'api'))
                if config.has_option('bitcoin', 'network'):
                    self.network_var.set(config.get('bitcoin', 'network'))
                
                # Configuration Tor
                if config.has_option('tor', 'enabled'):
                    self.tor_var.set(config.getboolean('tor', 'enabled'))
                if config.has_option('tor', 'host'):
                    self.tor_host.set(config.get('tor', 'host'))
                if config.has_option('tor', 'port'):
                    self.tor_port.set(config.get('tor', 'port'))
                
                # Configuration Bitcoin Core
                if config.has_option('bitcoincore', 'rpc_user'):
                    self.rpc_user.set(config.get('bitcoincore', 'rpc_user'))
                if config.has_option('bitcoincore', 'rpc_password'):
                    self.rpc_pass.set(config.get('bitcoincore', 'rpc_password'))
                
                self.log("✅ Configuration chargée depuis gateway.ini", "success")
                
            except Exception as e:
                self.log(f"⚠️ Erreur chargement config: {e}", "warning")
        else:
            self.log("ℹ️ Aucun fichier gateway.ini trouvé, utilisation des paramètres par défaut", "info")
    ...
    def auto_connect_mesh(self):
        """Connexion automatique au premier port disponible"""
        ports = [p.device for p in serial.tools.list_ports.comports()]
        self.log(f"🔍 Ports série détectés: {ports}", "info")
        if not ports:
            self.log("⚠️ Aucun port série trouvé - Mode démo activé", "warning")
            self.log("💡 Vous pouvez tester la diffusion Bitcoin sans matériel connecté", "info")
            self.mesh_status.configure(text="📡 Mesh: Mode Démo", foreground="#ffa500")
            return False
            
        # Essayer chaque port
        for port in ports:
            try:
                self.log(f"🔍 Test connexion automatique sur {port}...", "info")
                self.interface = meshtastic.serial_interface.SerialInterface(port)
                pub.subscribe(self.on_mesh_receive, "meshtastic.receive")
                
                self.connected = True
                self.port_var.set(port)  # Mettre à jour l'interface
                self.mesh_status.configure(text="📡 Mesh: Connecté ✓", foreground="#00ff88")
                self.connect_btn.configure(text="Déconnecter")
                
                node_info = self.interface.getMyNodeInfo()
                name = node_info.get('user', {}).get('shortName', 'N/A')
                self.log(f"✅ Connexion automatique réussie sur {port} (nœud: {name})", "success")
                self.log("🎧 En écoute des transactions Bitcoin sur le mesh...", "info")
                return True
                
            except Exception as e:
                self.log(f"❌ Échec sur {port}: {e}", "warning")
                continue
                
        self.log("⚠️ Impossible de se connecter au mesh - Mode démo activé", "warning")
        self.log("💡 Vous pouvez tester la diffusion Bitcoin sans matériel connecté", "info")
        self.mesh_status.configure(text="📡 Mesh: Mode Démo", foreground="#ffa500")
        return False
    ...
    def setup_tor_auto(self):
        """Configuration Tor pour le mode automatique (sans boîte de dialogue)"""
        if not TOR_AVAILABLE:
            self.log("⚠️ PySocks non installé - Mode clearnet", "warning")
            self.tor_var.set(False)
            self.session = requests.Session()
            return
        ...
        try:
            ...
            # Test connexion Tor
            self.log("🧅 Test connexion Tor...", "info")
            r = self.session.get("https://check.torproject.org/api/ip", timeout=10)
            data = r.json()
            if data.get("IsTor"):
                self.btc_status.configure(text="₿ Bitcoin: via Tor 🧅", foreground="#00ff88")
                self.log(f"✅ Connecté via Tor (IP: {data.get('IP', 'N/A')})", "success")
            ...
            self.log(f"⚠️ Tor indisponible: {e}", "warning")
            self.log("🌐 Bascule automatique en mode clearnet", "info")
            self.tor_var.set(False)
            self.session = requests.Session()
            self.btc_status.configure(text="₿ Bitcoin: Clearnet", foreground="#00aaff")
            return False
    ...
    def handle_btx_chunk(self, text, sender):
        ...
        try:
            ...
            parts = text.split(":", 2)  # Split en 3 parties max (BTX, n/total, data)
            self.log(f"[DEBUG] BTX split parts: {parts}", "info")
            if len(parts) < 3:
            ...
            self.log(f"[DEBUG] BTX chunk data: {chunk_data}", "info")
            ...
            if buffer["total"] != total_chunks:
                buffer["chunks"] = {}
                buffer["total"] = total_chunks
            buffer["chunks"][chunk_num] = chunk_data
            buffer["last_time"] = time.time()
            # Assembler toutes les parties
            full_data = "".join([buffer["chunks"].get(i+1,"") for i in range(total_chunks)])
            self.log(f"[DEBUG] BTX full_data: {full_data}", "info")
            # Vérifier si c'est du hex
            if not all(c in '0123456789abcdefABCDEF' for c in full_data):
                self.log(f"❌ Chunk data non hexadécimal: {full_data[:50]}...", "error")
                return
            self.log(f"✅ BTX chunk(s) reçus et valides !", "success")
        except Exception as e:
            self.log(f"Exception BTX: {e}", "error")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bitcoin Mesh Gateway")
    parser.add_argument("--auto", action="store_true", help="Connexion automatique au mesh au démarrage")
    parser.add_argument("--tor", action="store_true", help="Activer Tor automatiquement")
    parser.add_argument("--api", choices=["Mempool.space", "Blockstream", "Bitcoin Core (local)"], 
                       default="Mempool.space", help="API Bitcoin par défaut")
    parser.add_argument("--network", choices=["mainnet", "testnet"], 
                       default="mainnet", help="Réseau Bitcoin")
    ...
    # Configuration automatique si demandée
    if args.auto:
        app.log("🚀 Mode automatique activé", "info")
        app.log("🔍 Recherche de ports série pour connexion mesh...", "info")
        # Attendre que l'interface soit prête
        root.after(1000, lambda: app.auto_connect_mesh())
        
    if args.tor:
        app.log("🧅 Activation automatique de Tor", "info")
        root.after(1500, lambda: app.setup_tor_auto())
        
    if args.api != "Mempool.space":
        app.api_var.set(args.api)
        
    if args.network != "mainnet":
        app.network_var.set(args.network)
